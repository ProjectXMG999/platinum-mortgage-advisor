...
Zrefaktoryzowany - wykorzystuje serwisy modułowe
"""
...
class AIClient:
    ...
    def _init_services(self):
        """Inicjalizuje moduły serwisowe z zoptymalizowaną architekturą"""
        # Context loader - dynamiczne ładowanie danych z separacją WYMÓG/JAKOŚĆ
        self.context_loader = ContextLoader()
        
        # Prompt loader - budowanie messages z dynamicznym kontekstem
        self.prompt_loader = PromptLoader(
            prompts_dir="src/prompts",
            context_loader=self.context_loader
        )
        
        # Validation service (ETAP 1 - tylko WYMÓG)
        self.validation_service = ValidationService(
            ai_client=self,
            prompt_loader=self.prompt_loader
        )
        
        # Quality service (ETAP 2 - tylko JAKOŚĆ)
        self.quality_service = QualityService(
            ai_client=self,
            prompt_loader=self.prompt_loader
        )
        
        # Ranking service (ETAP 3 - szczegółowy TOP 3)
        self.ranking_service = RankingService(
            ai_client=self,
            prompt_loader=self.prompt_loader
        )
        
        # Orchestrator service (koordynacja wszystkich etapów)
        self.orchestrator = OrchestratorService(
            validation_service=self.validation_service,
            quality_service=self.quality_service
        )
        
        print("✓ Zainicjalizowano serwisy: ContextLoader, PromptLoader, ValidationService, QualityService, RankingService, Orchestrator")
        
        # Statystyki
        num_banks = len(self.context_loader.knowledge_base)
        num_wymog = self.context_loader.classification.get('statistics', {}).get('WYMÓG_count', 0)
        num_jakosc = self.context_loader.classification.get('statistics', {}).get('JAKOŚĆ_count', 0)
        
        print(f"  - ContextLoader: {num_banks} banków, {num_wymog} WYMOGÓW, {num_jakosc} JAKOŚCI")
...
"""
    ...
    async def validate_requirements_async(
        ...
        ZOPTYMALIZOWANE: Używa ValidationService z ContextLoader
        
        Args:
            user_query: Zapytanie użytkownika (profil klienta) - DEPRECATED
            knowledge_base: Pełna baza wiedzy - DEPRECATED (używa ContextLoader)
        ...
        Returns:
            Tuple (odpowiedź JSON jako string, parsed dict)
        """
        # Deleguj do ValidationService (używa nowych serwisów z ContextLoader i structured outputs)
        return await self.validation_service.validate_all_banks(
            knowledge_base=knowledge_base,  # Deprecated - ignorowane
    ...
    async def query_three_stage_optimized(
        self,
        customer_profile,
        etap1_model: str = None,
        etap2_model: str = None,
        etap3_model: str = None,
        top_n: int = 3
    ):
        """
        🚀 ZOPTYMALIZOWANY 3-ETAPOWY PIPELINE z Structured Outputs
        
        Wykorzystuje nową architekturę z ContextLoader i ResponseParser:
        - ETAP 1: Walidacja WYMOGÓW (tylko 68 parametrów WYMÓG)
        - ETAP 2: Ocena JAKOŚCI (tylko 19 parametrów JAKOŚĆ)
        - ETAP 3: Szczegółowy ranking TOP 3 (pełny kontekst)
        
        Args:
            customer_profile: CustomerProfile object (zmapowany profil)
            etap1_model: Model dla ETAP 1 (None = domyślny)
            etap2_model: Model dla ETAP 2 (None = domyślny)
            etap3_model: Model dla ETAP 3 (None = domyślny)
            top_n: Liczba banków w finalnym rankingu (domyślnie 3)
            
        Returns:
            CompleteAnalysis object z pełną analizą
        """
        import time
        start_time = time.time()
        
        print("\n" + "="*80)
        print("🚀 ZOPTYMALIZOWANY 3-ETAPOWY PIPELINE (Structured Outputs)")
        print("="*80)
        print(f"📊 ContextLoader: {len(self.context_loader.knowledge_base)} banków")
        print(f"📋 CustomerProfile: {customer_profile.to_dict()}")
        print("="*80 + "\n")
        
        # ETAP 1: Walidacja WYMOGÓW (parallel, tylko WYMÓG context)
        print("🔍 ETAP 1: Walidacja WYMOGÓW...")
        validation_results = await self.validation_service.validate_all_banks(
            knowledge_base=None,  # Deprecated - używa ContextLoader
            customer_profile=customer_profile,
            deployment_name=etap1_model,
            show_progress=True
        )
        
        # Filtruj PASSED banki
        passed_banks = [r for r in validation_results if r.status == "PASSED"]
        print(f"✅ Zakwalifikowane: {len(passed_banks)}/{len(validation_results)} banków\n")
        
        if not passed_banks:
            print("❌ Brak banków spełniających wymagania!")
            from src.models.structured_outputs import CompleteAnalysis, DetailedRanking
            return CompleteAnalysis(
                ...
                detailed_ranking=DetailedRanking(
                    top_banks=[],
                    comparison_table="Brak banków do porównania",
                    recommendation="Brak banków spełniających wymagania klienta",
                    full_markdown="# Brak wyników\n\nŻaden bank nie spełnia podstawowych wymogów."
                ),
                processing_time=time.time() - start_time
            )
        
        # ETAP 2: Ocena JAKOŚCI (parallel, tylko JAKOŚĆ context)
        print("🏅 ETAP 2: Ocena JAKOŚCI...")
        passed_bank_names = [r.bank_name for r in passed_banks]
        
        # Uruchom quality scoring
        quality_tasks = [
            self.quality_service.rate_single_bank(
                bank_name=bank_name,
                customer_profile=customer_profile,
                deployment_name=etap2_model
            )
            for bank_name in passed_bank_names
        ]
        
        quality_scores = await asyncio.gather(*quality_tasks)
        
        # Sortuj po total_score
        quality_scores.sort(key=lambda x: x.total_score, reverse=True)
        print(f"✅ Oceniono: {len(quality_scores)} banków")
        
        # Wyświetl TOP N
        for i, score in enumerate(quality_scores[:top_n], 1):
            print(f"  {i}. {score.bank_name}: {score.total_score}/100 pkt")
        print()
        
        # ETAP 3: Szczegółowy ranking TOP N (full context)
        print(f"🎯 ETAP 3: Szczegółowy ranking TOP {top_n}...")
        top_bank_names = [s.bank_name for s in quality_scores[:top_n]]
        
        detailed_ranking = await self.ranking_service.generate_detailed_ranking(
            top_banks=top_bank_names,
            customer_profile=customer_profile,
            deployment_name=etap3_model
        )
        
        print(f"✅ Wygenerowano szczegółowy ranking\n")
        
        # Stwórz CompleteAnalysis
        from src.models.structured_outputs import CompleteAnalysis
        ...
        print("="*80)
        print(f"✅ PIPELINE ZAKOŃCZONY ({complete_analysis.processing_time:.2f}s)")
        print("="*80)
